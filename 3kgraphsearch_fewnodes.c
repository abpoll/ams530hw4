...
float calcA(int dist[nnodes][nnodes]){
  int i, j;
  float a = 0;
...
float starttime, endtime; /*Calculating runtime*/
MPI_Status status;
...
num_adjs = 280; /*Total number of adjacency matrices. Can adjust */
...
twokadj[nnodes-1][0] = 1;
   ...
   */
  //This is a copy of twokadj that will be filled each time
  int threekadj[nnodes][nnodes];
  memset(threekadj, 0, nnodes*nnodes*sizeof(int));
  int diam;
  float avg;
  int threekadjbest[nnodes][nnodes];
  memset(threekadjbest, 0, nnodes*nnodes*sizeof(int));
  int diambest = 100; //arbitrary large for comparison. adjust for larger nnodes
  float avgbest = 100; //arbitrary large for comparison. adjust for larger nnodes
  
  starttime = MPI_Wtime(); /*Start timing*/
  
  for(i = 0; i < adjs; i++){
    //Reset threekadj
    memcpy(threekadj, twokadj, sizeof(twokadj));
    
    // Reset the vertex list
    int v[nnodes];
    memset(v, 0, nnodes*sizeof(int));
    int sum = 0;
    for(j = 0; j < nnodes; j++){
      v[j] = j;
      sum += v[j];
    ...
    if(diam<=diambest && avg<=avgbest){
      diambest = diam;
      avgbest = avg;
      memcpy(threekadjbest, threekadj, sizeof(threekadj));  
    }
